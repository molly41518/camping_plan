    ...
    public class Sale : IDisposable
    {
        ...
        private int GetSaleAmount(DateTime startDate, DateTime endDate)
        {
            using (dbcon db = new dbcon())
            {
                ...
                {
                    object obj_value = db.order_detail
                        .Join(db.order, p => p.order_no, d => d.order_no,
                        (p1, d1) => new { p1, d1 })
                ...
                if (UserAccount.RoleNo == AppEnums.enUserRole.Member)
                {
                    object obj_value = db.order_detail
                        ...
                        .Where(m => m.d1.order_date >= startDate)
                        .Where(m => m.d1.order_date <= endDate)
                        .Where(m => m.d1.mno == UserAccount.UserNo)
                        .Sum(m => m.p1.amount);
                    if (obj_value != null) int.TryParse(obj_value.ToString(), out int_amount);
                }
        ...
        private List<cvmSaleRank> GetSaleAmountRank(DateTime startDate, DateTime endDate)
        {
            using (dbcon db = new dbcon())
            {
                ...
                if (UserAccount.RoleNo == AppEnums.enUserRole.Admin)
                {
                    List<cvmSaleRank> model1 = db.order_detail
                        .Join(db.order, p => p.order_no, d => d.order_no,
                        (p1, d1) => new { p1, d1 })
                        .Where(m => m.d1.order_date >= startDate)
                        .Where(m => m.d1.order_date <= endDate)
                        .Where(m => m.d1.order_validate == 1)
                        .GroupBy(m => new { m.p1.pno, m.p1.pname })
                        .Select(m => new cvmSaleRank
                        {
                            RankNumber = 0,
                            ProductNo = m.Key.pno,
                            ProductName = m.Key.pname,
                ...
                if (UserAccount.RoleNo == AppEnums.enUserRole.Member)
                {
                    List<cvmSaleRank> model2 = db.order_detail
                        .Join(db.order, p => p.order_no, d => d.order_no,
                        (p1, d1) => new { p1, d1 })
                        .Where(m => m.d1.order_date >= startDate)
                        .Where(m => m.d1.order_date <= endDate)
                ...
                if (UserAccount.RoleNo == AppEnums.enUserRole.Vendor)
                {
                    List<cvmSaleRank> model3 = db.order_detail
                        .Join(db.order, p => p.order_no, d => d.order_no,
                        (p1, d1) => new { p1, d1 })
                        .Where(m => m.d1.order_date >= startDate)
                        .Where(m => m.d1.order_date <= endDate)
